...
<style>
    ...
    .sidebar-brand h2 {
        ...
        color: var(--accent-sapphire) !important;
        margin-bottom: 8px;
    }
    .sidebar-eyebrow {
...
@st.cache_data(ttl=3600)
def get_selic_monthly_end(days=730):
    """Retorna a Selic Meta no último dia útil de cada mês (para alinhar com IPCA 12M mensal)."""
    try:
        ...
        start_date = datetime.now() - timedelta(days=days)
        df = df[df['data'] >= start_date].sort_values('data')
        # Resample para pegar o último valor de cada mês
        df = df.set_index('data').resample('ME').last().dropna().reset_index()
        df['Date'] = df['data'].dt.date
...
with tab1:
    ...
    with col2:
        # ===== BOTÕES DE ANÁLISE MACRO =====
        macro_view = st.radio(
            "Análise Macro",
            ["Selic vs IPCA 12M", "Juro Real (Spread)", "Spread CDI-Selic", "IPCA Mensal vs Acum. 12M"],
            index=0,
            horizontal=True,
            label_visibility="collapsed"
        )

        # ===== OPÇÃO 1: Selic vs IPCA Acumulado 12M (CORRIGIDO - série 13522) =====
        if macro_view == "Selic vs IPCA 12M":
            st.markdown("<p class='section-label'>Spread histórico: Selic vs IPCA Acumulado (12 meses)</p>", unsafe_allow_html=True)

            # ✅ CORREÇÃO: série 13522 = IPCA Variação acumulada em 12 meses (a 10844 era ERRADA)
            df_selic_h = get_historical_macro(432, "SELIC", days=365)
            df_ipca_h  = get_historical_macro(13522, "IPCA Acumulado", days=365)
            ...
            # Anotação com o Juro Real atual (calculado globalmente)
        ...
        # ===== OPÇÃO 2: Juro Real Histórico (Spread Selic - IPCA 12M) =====
        elif macro_view == "Juro Real (Spread)":
            st.markdown("<p class='section-label'>Juro Real Histórico: Selic − IPCA Acum. 12M</p>", unsafe_allow_html=True)

            df_selic_m = get_selic_monthly_end(days=730)
            df_ipca_12m = get_historical_macro(13522, "IPCA12M", days=730)

            if not df_selic_m.empty and not df_ipca_12m.empty:
                df_merged = pd.merge(df_selic_m, df_ipca_12m, on='Date', how='inner')
                if not df_merged.empty:
                    df_merged['Juro_Real'] = (
                        ((1 + df_merged['SELIC'] / 100) / (1 + df_merged['IPCA12M'] / 100)) - 1
                    ) * 100

                    colors = ['#3fb950' if v >= 0 else '#f85149' for v in df_merged['Juro_Real']]

                    fig_jr = go.Figure()
                    fig_jr.add_trace(go.Bar(
                        x=df_merged['Date'],
                        y=df_merged['Juro_Real'],
                        name='Juro Real (%)',
                        marker_color=colors,
                        opacity=0.85
                    ))

                    # Linha da Selic para referência
                    ...
                    fig_jr.add_hline(y=0, line_dash="dash", line_color="gray", opacity=0.6)

                    if not is_missing(juro_real):
                        data_atual = datetime.now().strftime("%d/%m/%Y")
                        fig_jr.add_annotation(
                            x=0.98, y=0.98, xref="paper", yref="paper",
                            text=f"Juro Real: {juro_real:.2f}% ({data_atual})",
                            showarrow=False,
                            font=dict(color="white", size=11, family="JetBrains Mono"),
                            bgcolor="rgba(0,0,0,0.7)", bordercolor="white", borderwidth=1,
                            borderpad=4, opacity=0.9, align="right"
                    ...
                    fig_jr.update_layout(
                        ...
                        margin=dict(l=10, r=10, t=10, b=10), height=300,
                        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
                        yaxis=dict(title="Juro Real (%)", side="left"),
                        yaxis2=dict(title="Selic (%)", overlaying="y", side="right", showgrid=False),
                        barmode='relative'
                    )
                    st.plotly_chart(fig_jr, use_container_width=True)
                else:
                    st.warning("Não foi possível alinhar as datas entre Selic e IPCA 12M.")
            else:
                st.warning("Dados insuficientes para calcular o Juro Real histórico.")

        # ===== OPÇÃO 3: Spread CDI-Selic =====
        elif macro_view == "Spread CDI-Selic":
            st.markdown("<p class='section-label'>Spread CDI − Selic Meta (mensal)</p>", unsafe_allow_html=True)

            df_spread = get_historical_macro(4390, "Spread_CDI_Selic", days=730)

            if not df_spread.empty:
                colors_sp = ['#3fb950' if v >= 0 else '#f85149' for v in df_spread['Spread_CDI_Selic']]

                fig_sp = go.Figure()
                fig_sp.add_trace(go.Bar(
                    x=df_spread['Date'],
                    y=df_spread['Spread_CDI_Selic'],
                    name='Spread CDI-Selic (p.p.)',
                    marker_color=colors_sp,
                    opacity=0.85
                ))
                fig_sp.add_hline(y=0, line_dash="dash", line_color="gray", opacity=0.6)

                # Anotação com último valor
                ultimo_spread = df_spread['Spread_CDI_Selic'].iloc[-1]
                fig_sp.add_annotation(
                    x=0.98, y=0.98, xref="paper", yref="paper",
                    text=f"Último: {ultimo_spread:.2f} p.p.",
                    showarrow=False,
                    font=dict(color="white", size=11, family="JetBrains Mono"),
                    bgcolor="rgba(0,0,0,0.7)", bordercolor="white", borderwidth=1,
                    borderpad=4, opacity=0.9, align="right"
                ...
                fig_sp.update_layout(
                    ...
                    margin=dict(l=10, r=10, t=10, b=10), height=300,
                    legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
                    yaxis=dict(title="Spread (pontos percentuais)")
                )
                st.plotly_chart(fig_sp, use_container_width=True)
            else:
                st.warning("Dados indisponíveis para o Spread CDI-Selic.")

        # ===== OPÇÃO 4: IPCA Mensal vs Acumulado 12M =====
        elif macro_view == "IPCA Mensal vs Acum. 12M":
            st.markdown("<p class='section-label'>IPCA: Variação Mensal vs Acumulado em 12 Meses</p>", unsafe_allow_html=True)

            df_ipca_mensal = get_historical_macro(433, "IPCA_Mensal", days=730)
            df_ipca_12m_v = get_historical_macro(13522, "IPCA_12M", days=730)

            fig_ip = go.Figure()

            if not df_ipca_mensal.empty:
                fig_ip.add_trace(go.Bar(
                    x=df_ipca_mensal['Date'],
                    y=df_ipca_mensal['IPCA_Mensal'],
                    name='IPCA Mensal (%)',
                    marker_color='#d15704',
                    opacity=0.7,
                    yaxis='y'
            ...
            if not df_ipca_12m_v.empty:
                fig_ip.add_trace(go.Scatter(
                    x=df_ipca_12m_v['Date'],
                    y=df_ipca_12m_v['IPCA_12M'],
                    name='IPCA Acum. 12m (%)',
                    line=dict(color='#f9826c', width=2.5),
                    yaxis='y2'
                ))

            fig_ip.add_hline(y=0, line_dash="dash", line_color="gray", opacity=0.4)

            # Faixa da meta de inflação (3% +/- 1.5p.p.)
            fig_ip.add_hrect(y0=1.5, y1=4.5, line_width=0, fillcolor="rgba(77, 142, 255, 0.06)", annotation_text="Meta (1.5% – 4.5%)")
            ...
            fig_ip.update_layout(
                ...
                margin=dict(l=10, r=10, t=10, b=10), height=300,
                legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
                yaxis=dict(title="IPCA Mensal (%)", side="left"),
                yaxis2=dict(title="IPCA Acum. 12M (%)", overlaying="y", side="right", showgrid=False),
                barmode='relative'
            )
    ...
    if not df_all_stocks.empty:
        ...
        fig_comp.update_layout(
            template="plotly_dark",
            paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)',
            margin=dict(l=10, r=10, t=15, b=10), height=450,
